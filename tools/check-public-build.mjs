...
try {
  ...
  if (existsSync(indexPath)) {
    ...
    if (!html.includes('tiinex:build-id') || !html.includes('CP328-route-render-placement-foundation')) fail('public index must include CP328 build identity meta.');
  ...
  if (existsSync(bundlePath)) {
    ...
    if (!bundle.includes("release: '328'") || !bundle.includes('buildIdentityReport') || !bundle.includes('routeLoadPresentationReport')) fail('public bundle must include release 328 build and route-load diagnostics.');
    const sections = [
  ...
  console.log('✓ public build preserves CNAME, favicon, and CP328 build identity');
} finally {
