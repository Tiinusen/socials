...
try {
  ...
  if (existsSync(indexPath)) {
    ...
    if (!html.includes('tiinex:build-id') || !html.includes('CP334-mobile-density-lifecycle-export-binding')) fail('public index must include CP334 build identity meta.');
  ...
  if (existsSync(bundlePath)) {
    ...
    if (!bundle.includes("release: '334'") || !bundle.includes('buildIdentityReport') || !bundle.includes('routeLoadPresentationReport')) fail('public bundle must include release 334 build and route-load diagnostics.');
    const sections = [
  ...
  console.log('✓ public build preserves CNAME, favicon, and CP334 build identity');
} finally {
