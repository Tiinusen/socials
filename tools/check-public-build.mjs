...
try {
  ...
  if (existsSync(indexPath)) {
    ...
    if (!html.includes('tiinex:build-id') || !html.includes('CP329-placement-picker-foundation')) fail('public index must include CP329 build identity meta.');
  ...
  if (existsSync(bundlePath)) {
    ...
    if (!bundle.includes("release: '329'") || !bundle.includes('buildIdentityReport') || !bundle.includes('routeLoadPresentationReport')) fail('public bundle must include release 329 build and route-load diagnostics.');
    const sections = [
  ...
  console.log('✓ public build preserves CNAME, favicon, and CP329 build identity');
} finally {
