...
try {
  ...
  if (existsSync(indexPath)) {
    ...
    if (!html.includes('tiinex:build-id') || !html.includes('CP331-mobile-reclaim-parent-scope')) fail('public index must include CP331 build identity meta.');
  ...
  if (existsSync(bundlePath)) {
    ...
    if (!bundle.includes("release: '331'") || !bundle.includes('buildIdentityReport') || !bundle.includes('routeLoadPresentationReport')) fail('public bundle must include release 331 build and route-load diagnostics.');
    const sections = [
  ...
  console.log('✓ public build preserves CNAME, favicon, and CP331 build identity');
} finally {
