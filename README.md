...
Transport badges show the effective tier for the current source load. Cache restores present cached repository and issue material as `cache` regardless of the original live tier that produced the cache entry; user clicks replace the current tier instead of accumulating stale badges. Time Portal is a commit-pinned historical Git-state mode, so it bypasses the normal latest-state `cache`/`mirror` tiers and promotes those requests to `proxy`; `direct` remains the explicit last-resort raw fallback. Historical direct reads use the immutable per-file cache and read budget before raw GETs, and Tiinex/docs freshness supplements are disabled for historical snapshots so Time Portal does not expand into unrelated schema/root probes. Issue/comment material remains timestamp-filtered observation material.
...
### Issue publication scheduling

Hosted issue snapshot publication is cooldown/coalesced. The workflow serializes public-branch writers, reads `.tiinex/issue-publish-state.json` from the current public artifact, and publishes immediately when the cooldown has elapsed. If the previous publication was too recent, it waits only the remaining time and then reconciles the latest complete GitHub issue/comment state. New issue events do not reset the timer, and `cancel-in-progress` is disabled for the public-branch publication group to avoid starvation.
