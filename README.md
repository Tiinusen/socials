...
Transport badges show the effective tier for the current source load. Cache restores present cached repository and issue material as `cache` regardless of the original live tier that produced the cache entry; user clicks replace the current tier instead of accumulating stale badges. Time Portal route restores are cache-first and network-silent: an exact historical source-material cache may restore the commit-pinned repository-file view, while a missing cache records that explicit direct refresh is needed instead of silently issuing raw requests. User-invoked direct Time Portal refresh uses seeded artifact paths and immutable per-file cache before raw GETs; issue/comment material remains timestamp-filtered observation material.
