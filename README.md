...
- `TIINEX_ISSUE_SNAPSHOTS`: defaults to `true`. During site publish, the workflow exports public GitHub issues/comments from the publishing repository into `issues/github.com/<owner>/<repo>/` so hosted viewers list and read same-origin issue material before live GitHub.
